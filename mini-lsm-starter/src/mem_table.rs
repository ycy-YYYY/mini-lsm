...
impl MemTable {
    ...
    pub fn scan(&self, _lower: Bound<&[u8]>, _upper: Bound<&[u8]>) -> MemTableIterator {
        ...
        iter.with_item_mut(|item| *item = entry);
        iter
    }
    ...
    pub fn flush(&self, _builder: &mut SsTableBuilder) -> Result<()> {
        for entry in self.map.iter() {
            _builder.add(KeySlice::from_slice(&entry.key()), entry.value().as_ref());
        }
        Ok(())
    }
...
impl MemTableIterator {
    /// generate a key-value pair from an entry, if the entry is None, return an empty key-value pair.
